...
def is_bank_holiday(event_data: Dict) -> bool:
    return "bank holiday" in event_data.get("event", "").lower()


def is_usd_high_impact(event_data: Dict) -> bool:
    return (
...
def get_argentina_business_hours(month: int) -> Tuple[int, int, int, int]:
    if 3 <= month <= 11:
        return 10, 30, 13, 0
    else:
        return 11, 30, 14, 0
...
def main():
    ...
    result = parse_embedded_calendar(html, date_obj)
    if "error" in result:
    ...
    bank_holidays = [e for e in all_events if is_bank_holiday(e)]

    usd_high = [e for e in all_events if is_usd_high_impact(e)]

    start_h, start_m, end_h, end_m = get_argentina_business_hours(
        date_obj.month
    )

    high_impact = filter_by_time_range(usd_high, start_h, start_m, end_h, end_m)
